...
class ExamStartView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, {
            ...
            'score_speaking_pct': round(score_speaking / 25 * 100),
            'score_listening_pct': round(score_listening / 25 * 100),
            'score_choice_pct': round(score_choice / 25 * 100),
            'score_writing_pct': round(score_writing / 25 * 100),
            'writing_pending': writing_pending,
            'tipo': tipo,
            'nivel_activo': nivel_activo,
            'nivel_siguiente': nivel_siguiente,
